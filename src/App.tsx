...
import { Mail, ExternalLink, Code2, Briefcase, GraduationCap, Trophy, ChevronRight, MessageSquare, Download, Languages } from 'lucide-react';
...
const translations = {
  en: {
      ...
      roles: [
        {
          ...
          company: "University of Guadalajara - General Rector's Office",
    ...
    achievements: {
      ...
      desc1: "Active participant in ICPC"
    },
    footer: "Built with React & Purple."
  },
  es: {
    nav: {
    ...
    achievements: {
      title: "Logros",
      title1: "Competitive Programming",
      desc1: "Participante activo en ICPC"
    },
    footer: "Hecho con React & Morado."
  }
};
    ...
    <div className="min-h-screen bg-[#080808] text-zinc-200 selection:bg-purple-500/30 selection:text-purple-200 relative overflow-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#080808]/80 backdrop-blur-md border-b border-zinc-800">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          >
            Rivas<span className="text-purple-500">.</span>dev
          </motion.span>
      ...
      <main className="max-w-5xl mx-auto px-6 pt-32 pb-20">
        ...
        >
          ...
          <div className="flex flex-wrap gap-4">
            <a href="https://linkedin.com/in/fernando-rivas2003/" target="_blank" className="flex items-center gap-2 px-5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-full hover:border-purple-500 transition-all group">
              <Briefcase size={18} className="group-hover:text-purple-500" />
              <span>LinkedIn</span>
            </a>
            <a href="https://github.com/Rivas0810" target="_blank" className="flex items-center gap-2 px-5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-full hover:border-purple-500 transition-all group">
              <Code2 size={18} className="group-hover:text-purple-500" />
              <span>GitHub</span>
            </a>
            <a href="mailto:fernandorivasj03@gmail.com" className="flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white rounded-full hover:bg-purple-700 transition-all font-medium">
              <Mail size={18} />
              <span>{t.hero.contact}</span>
            </a>
            <a href="https://wa.me/523311509928?text=Hola%20Carlos,%20me%20gustaría%20ponerme%20en%20contacto%20contigo" target="_blank" className="flex items-center gap-2 px-5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-full hover:border-purple-500 transition-all group">
              <MessageSquare size={18} className="group-hover:text-purple-500" />
              <span>{t.hero.whatsapp}</span>
            </a>
            <a href="/cv.pdf" download className="flex items-center gap-2 px-5 py-2.5 bg-zinc-900 border border-zinc-800 rounded-full hover:border-purple-500 transition-all group">
          ...
          >
            {t.experience.roles.map((role, i) => (
              <motion.div key={i} variants={fadeIn} className="relative group">
                ...
                <div className="p-8 rounded-2xl bg-zinc-900/30 border border-zinc-800 hover:border-purple-500/50 transition-all backdrop-blur-sm">
                  <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6">
                    <div>
                      <h3 className="text-2xl font-bold text-white mb-1">{role.title}</h3>
                      <p className="text-purple-500 font-semibold text-lg">{role.company}</p>
                    </div>
                    <span className="text-zinc-500 text-sm font-mono mt-1 md:mt-0 px-3 py-1 bg-zinc-900 rounded-full border border-zinc-800">{role.date}</span>
                  </div>
                  <ul className="space-y-4 text-zinc-400 leading-relaxed text-lg">
